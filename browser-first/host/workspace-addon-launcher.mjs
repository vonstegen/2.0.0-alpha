...
export async function startWorkspaceAddons({
  ...
  for (const entry of workspaceAddonManifests) {
    ...
    // SDK-DEMO-002-FIX-2: the bridge token is NOT exposed to addon
    // upstreams. The renderer mints per-addon capability tokens via
    // the bridge's /api/capability-tokens endpoint and delivers them
    // to the iframe via postMessage; the add-on never needs the
    // bridge token (its /api/<addon>/* calls are same-origin and
    // authenticated with the capability token, enforced at the
    // upstream). We deliberately do NOT pass the bridge token into
    // the addon env.
    const env = {
      ...(parentEnv ?? {}),
