...
- Selected source-file intake now deduplicates requests, caps host batches, rejects symlink/path-escape selections, writes source version manifests atomically, records intake artifacts only against the latest reserved source version, stores content-addressed immutable source snapshots, and rolls back reserved source versions if intake artifact creation fails.
