...
- The Artifacts workspace now extracts source-specific preview metadata for browser page captures, selected text, AI page summaries, research trails, wallet/DAO audits, and browser job reports without exposing raw filesystem paths in chat.
...
- Living Archive review-queue cards now include an Inspect Source action that previews the preserved intake artifact in the same review flow before draft, verify, and promote.
...
- Review queue and promotion history can preview the promoted AI Memory markdown page through a scoped `AI_MEMORY/wiki` read route, so the user can inspect what actually became trusted memory without opening arbitrary files.
...
- Selected source-file intake now deduplicates requests, caps host batches, rejects symlink/path-escape selections, writes source version manifests atomically, records intake artifacts only against the latest reserved source version, stores content-addressed immutable source snapshots, verifies snapshot bytes before reuse, and rolls back reserved source versions if intake artifact creation fails.
- Move-on-import uses host-only preflight approval, content-hash fingerprint binding, move-time byte rechecks, destination hash verification, rollback ledgering, manifest-level rollback status, partial-failure rollback, conflict-reporting guarded rollback, empty-directory/root restoration, zero-skipped file/directory/root rollback deregistration, and non-destructive source cleanup that preserves unexpected new source files instead of recursively deleting the old source root.
...
- The page-overlay stop control now routes through a dedicated side-panel message router with deterministic coverage, so overlay cancellation remains testable instead of being hidden in the side-panel composition root.
...
- Latest restricted sandbox checkpoint on 2026-06-01: `git diff --check && npm run test:browser-first` passed with 374 tests and 8 explicit localhost-bridge skips caused by sandbox bind restrictions.
...
- Move-on-import with explicit confirmation, approved content-hash-backed preflight fingerprint matching, move-time source byte rechecks, managed-memory canonicalization, SHA-256 destination verification, ledgered rollback, empty-directory/root preservation, and automatic rollback on partial failure.
...
- Augmentor's browser-first system contract explicitly treats current/latest news and web research as Agent Control work if routing ever reaches the model, preventing fallback claims that Augmentor is only a text assistant or lacks internet access.
