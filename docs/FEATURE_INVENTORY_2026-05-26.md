...
- Move-on-import uses host-only preflight approval, content-hash fingerprint binding, move-time byte rechecks, destination hash verification, rollback ledgering, manifest-level rollback status, partial-failure rollback, guarded rollback, and non-destructive source cleanup that preserves unexpected new source files instead of recursively deleting the old source root.
