...
- Move-on-import must never silently destroy source data: it requires explicit confirmation, binds execution to the approved content-hash-backed preflight fingerprint, rechecks source bytes immediately before each file move, writes a fingerprinted manifest and ledger, verifies destination hashes before registration, never recursively deletes unexpected new source content during cleanup, automatically rolls back earlier moved files after partial failure, and only registers a source when the host move status is fully `moved`.
...
- Living Archive workspace single-file intake exists for supported text-like documents; unsupported media/binary files create metadata-only attachment stubs instead of trusted content.
- Move-on-import is host-mediated and guarded by content-hash-backed preflight fingerprint matching, exact confirmation phrase, source byte rechecks, destination byte-hash verification, rollback ledger, partial-failure rollback, and rollback hash checks.
...
- Connected Source Review can create governed intake from new/changed compatible files in explicit host-capped batches while skipping unchanged files and showing deferred counts.
...
- Needs real-data validation against large user vaults and provider-routed AI Memory builds.
