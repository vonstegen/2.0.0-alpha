...
- Move-on-import must never silently destroy source data: it requires explicit confirmation, binds execution to the approved preflight fingerprint, writes a manifest/ledger, verifies destination hashes before registration, automatically rolls back earlier moved files after partial failure, and only registers a source when the host move status is fully `moved`.
...
- Move-on-import is host-mediated and guarded by preflight fingerprint matching, exact confirmation phrase, byte-hash verification, rollback ledger, partial-failure rollback, and rollback hash checks.
