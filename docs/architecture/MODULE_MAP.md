...
| [`packages/addon-sdk-testing/`](../../packages/addon-sdk-testing/README.md) | In-process mock host + ADR-040 §7 F1–F10 negative-test harness for external-agent-runtime manifests; `permission-diff.ts` exports `diffAddOnManifest` per ADR-039; `trust-tier.ts` exports `getTrustTierFromManifest` and `canTransitionBetweenTiers` per ADR-042 | Optional test infrastructure; not a required Alpha host |
