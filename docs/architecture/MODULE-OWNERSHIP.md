...
| `browser-first/host/augmentor-consultation-*.mjs` | Read-only ontology consultation, neutral projection validation, selected-agent grants, answer policy, cache, deadlines, and metrics | Approved consumer projection and hashed access registry under external user state | Operator-local grants/revocations and in-memory consultation cache | Raw ontology sources, generated answers, memory writes, approval, routing, or authority decisions |
| `browser-first/host/run-augmentor-consultation-deployment.mjs` | Private-container HTTP adaptation for a deployment whose reverse proxy already verified fixed Basic Auth | Trusted proxy principal header and approved neutral projection | Consultation responses and in-memory cache only | Public authentication, TLS, password hashes, raw ontology sources, or any write operation |
