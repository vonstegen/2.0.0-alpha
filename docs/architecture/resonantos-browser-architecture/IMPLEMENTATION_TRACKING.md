...
| Extract generic start/status/events/cancel/artifact | `in-progress` | Phase 1 doc [`CP4-LIFECYCLE-EXTRACTION-MIGRATION.md`](CP4-LIFECYCLE-EXTRACTION-MIGRATION.md) + Phase 2 [`src/sdk/harnesses/hermes-provider-adapter.ts`](../../../src/sdk/harnesses/hermes-provider-adapter.ts) (lifted Hermes lifecycle: `localExecutionEnabled`, `resolveProvider`/`Model`, `credentialState`/`BlockedReason`, `startTask`/`diagnose`, `buildExecutionPrompt`/`parseCliOutput`, dependency-injected `HermesRuntime`); 17 parity tests in `hermes-provider-adapter.test.ts`; conformance suite still passes for all 8 reference providers; `engineer:verify` gate green; 1197 browser-first extension tests still pass |
