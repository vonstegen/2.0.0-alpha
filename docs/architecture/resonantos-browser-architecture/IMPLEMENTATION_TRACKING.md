...
| 98 | Migrate Hermes to generic adapter | `done` | end-to-end: live `run_agent.py` + bridge.startTask verified through `browser-first/test/hermes-bridge-end-to-end.test.mjs` (drives `createHermesProviderAdapterBridge().startTask` with `runtime.installed = true` against a fake venv; canonical fields + provider/model match). Existing in-process self-test covers the route path; the new test covers the bridge-direct path. |
| 99 | Migrate OpenCode + workspace lease/isolation | `done` | workspace lease wired into `opencodeRuntimeDispatch` (`browser-first/host/workspace-lease.mjs` registry + try/finally release); 12 lease tests + 1 e2e (mock opencode serve) + the Phase 4 cutover + 4 OpenClaw gateway tests in 5.2; the bridge cannot bypass the lease (403 `workspace-lease-unavailable` for unacquirable or contended leases). |
| 100 | Validate OpenClaw against the contract | `done` | real MCP-gateway transport added in `browser-first/host/openclaw-gateway-client.mjs` (`createOpenclawGatewayClient` + `openclawGatewayRuntimeDispatch`); `createOpenClawProviderAdapter` opts into the gateway when `options.gatewayClient` is provided. 4 e2e tests in `openclaw-gateway-e2e.test.mjs` prove: happy path forwards the child actor's response through the gateway; gateway 403 surfaces as dispatch deny; forged subject is denied BEFORE the gateway is contacted (gateway is the only authority path for the child actor). |
...
| 103 | Archive reads scoped/cited; writes intake-only | `done` | 8 tests in `per-harness-archive-parity.test.mjs` (Hermes, OpenCode, OpenClaw host-command + gateway, AgentZero, DeepSeek harness, Pi, Aider) prove the adapter layer never embeds a direct-write key (archivePath, wikiPath, memoryPath, etc.) even when the child-actor reply contains an `## Archive Citation` section. The intake path (`archive-review-service.executeArchiveIntake`) remains the only writer. |
