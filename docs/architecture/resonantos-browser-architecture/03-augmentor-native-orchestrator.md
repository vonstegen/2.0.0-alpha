...
## Compatibility with replaceable slots

The target SHOULD formalize `primary-agent` as an orchestration role or add an `orchestrator` slot alias through a compatible migration. Avoid introducing two competing concepts. A third-party primary agent must use the same SDK contracts and cannot inherit Augmentor's identity or grants.
