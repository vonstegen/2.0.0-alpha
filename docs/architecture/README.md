...
| [ADR-026: Minimal Kernel And Replaceable Default Add-ons](ADR-026-minimal-kernel-replaceable-default-addons.md) | Accepted | Applies | - | Core and add-ons | Replaceable chat, memory-system, and `primary-agent` boundaries apply; the `primary-agent` claim is governed by a host capability grant (see ADR-057). |
...
| [ADR-057: External Agent Runtime Boundary — ADR-056 ↔ #321 Reconciliation](ADR-057-external-agent-runtime-boundary-reconciliation.md) | Proposed | Applies | - | Core architecture | Policy floor that external agent runtimes are local-service add-ons with no raw credentials and no self-routing; ADR-053 is the architectural home; CP-5 partial implementation of #321 lives on `feat/cp5-phase5-reference-harnesses`. Reconciles vonstegen's (Andrew's) ADR-056 with this repo's governance; withdraws the `primary-agent` permanent-occupation pin (claimable via a host capability grant). |
