...
| [ADR-055: External Agent Runtime Boundary — ADR-056 ↔ #321 Reconciliation](ADR-055-external-agent-runtime-boundary-reconciliation.md) | Accepted | Applies | [ADR-053](ADR-053-browser-first-multi-harness-architecture.md) (architectural home), [ADR-018](ADR-018-addon-sdk-v0.md) (SDK contract) | Core architecture + maintainer (joint) | Policy floor that external agent runtimes are local-service add-ons with no raw credentials and no self-routing; ADR-053 is the architectural home; CP-5 partial implementation of #321 lives on `feat/cp5-phase5-reference-harnesses`. Reconciles the maintainer's ADR-056 with this repo's governance. |
